...
func main() {
	...
	for i := 0; i < len(triangles); i++ {
		...
		j := ((int(cx) | 0) + (int(cy) | 0) * width) << 2
		r, g, b := dst.Pix[j/4], dst.Pix[j/4+1], dst.Pix[j/4+2]
		fmt.Println(r, ":", g, ":", b)
		ctx.SetColor(color.RGBA{R:r, G:g, B:b, A:255})
		ctx.Fill()
	}
